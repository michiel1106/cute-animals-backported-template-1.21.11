package bikerboys.cuteanimalsport.code.cow;
...
public class CowRenderer extends MobRenderer<Cow, CowRenderState, CowModel> {
    ...
    private static Map<ModelType, AdultAndBabyModelPair<CowModel>> bakeModels(final EntityRendererProvider.Context context) {
    ...
    public void submit(final CowRenderState state, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final CameraRenderState camera) {
        if (state.variant != null) {
            this.model = (CowModel) ((AdultAndBabyModelPair)this.models.get(state.variant.modelAndTexture().model())).getModel(state.isBaby);
