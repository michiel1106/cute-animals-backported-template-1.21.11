package bikerboys.cuteanimalsport.code.chicken;
...
public class ChickenRenderer extends MobRenderer<Chicken, ChickenRenderState, ChickenModel> {
    ...
    private static Map<ModelType, AdultAndBabyModelPair<ChickenModel>> bakeModels(final EntityRendererProvider.Context context) {
    ...
    public void submit(final ChickenRenderState state, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final CameraRenderState camera) {
        if (state.variant != null) {
            this.model = (ChickenModel) ((AdultAndBabyModelPair)this.models.get(state.variant.modelAndTexture().model())).getModel(state.isBaby);
