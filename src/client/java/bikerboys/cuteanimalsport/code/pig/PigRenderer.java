...
public class PigRenderer extends MobRenderer<Pig, PigRenderState, PigModel> {
    ...
    private static Map<ModelType, AdultAndBabyModelPair<PigModel>> bakeModels(final EntityRendererProvider.Context context) {
    ...
    public void submit(final PigRenderState state, final PoseStack poseStack, final SubmitNodeCollector submitNodeCollector, final CameraRenderState camera) {
        if (state.variant != null) {
            this.model = (PigModel) ((AdultAndBabyModelPair)this.models.get(state.variant.modelAndTexture().model())).getModel(state.isBaby);
