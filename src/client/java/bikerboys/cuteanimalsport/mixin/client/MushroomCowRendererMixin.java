...
public class MushroomCowRendererMixin {
    ...
    @Unique
    private static final Identifier BROWN_COW_BABY = Constants.of("textures/entity/cow/mooshroom_brown_baby.png");
    @Unique
    private static final Identifier RED_COW_BABY = Constants.of("textures/entity/cow/mooshroom_red_baby.png");


    @WrapOperation(
            method = "<init>",
            at = @At(value = "NEW", target = "Lnet/minecraft/client/model/animal/cow/CowModel;", ordinal = 1)
    )
    ...
    @Inject(method = "getTextureLocation(Lnet/minecraft/client/renderer/entity/state/MushroomCowRenderState;)Lnet/minecraft/resources/Identifier;", at = @At("HEAD"), cancellable = true)
    private void getCustomTexture(MushroomCowRenderState state, CallbackInfoReturnable<Identifier> cir) {
        if (state.isBaby) {
